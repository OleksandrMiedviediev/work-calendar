...
export default function Home() {
  ...
  async function copyCalendarLink(calendar: SavedCalendar) {
    if (!calendar.webcalUrl) return notify("Ссылка пока недоступна.", "error");
    await navigator.clipboard?.writeText(calendar.webcalUrl);
    notify("Ссылка скопирована.");
  }

  async function shareCalendarLink(calendar: SavedCalendar) {
    if (!calendar.webcalUrl) return notify("Ссылка пока недоступна.", "error");
    if (navigator.share) {
      await navigator.share({ title: calendar.name, text: "Ссылка на календарь Work Calendar", url: calendar.webcalUrl });
      return;
    }
    await copyCalendarLink(calendar);
  }
      ...
      {view === "library" && savedCalendars.length > 0 && (
        <section className="saved-menu">
          <div className="saved-menu-heading">
            <div>
              <span className="section-kicker">{copy.library}</span>
              <h2>{copy.savedMonths}</h2>
            </div>
            <span className="saved-count">{savedCalendars.length} {copy.charts}</span>
          </div>
          <div className="saved-list">
            {[...savedCalendars].sort((a, b) => `${b.year || 0}-${b.month || 0}`.localeCompare(`${a.year || 0}-${a.month || 0}`)).map((calendar, index, all) => {
              ...
              return <div key={calendar.token}>{group !== previous && <div className="library-group">{calendar.month ? `${copy.month} ${String(calendar.month).padStart(2, "0")}.${calendar.year}` : copy.noDate}</div>}<div className="saved-row"><div className="saved-row-main"><span className="month-mark">{calendar.month ? String(calendar.month).padStart(2, "0") : "--"}</span><div><strong>{calendar.name}</strong><span className="saved-meta">{calendar.year || copy.noYear} · {calendar.eventCount} {copy.shifts}</span></div></div><div className="saved-row-actions"><button className="secondary compact" onClick={() => { loadSavedCalendar(calendar); setView("workspace"); }}>{copy.open}</button><button className="secondary compact" onClick={() => void copyCalendarLink(calendar)}>{copy.copyLink}</button><button className="secondary compact" onClick={() => void shareCalendarLink(calendar)}>{copy.share}</button><button className="danger-button" onClick={() => deleteSavedCalendar(calendar.token)} aria-label={`${copy.delete} ${calendar.name}`}>{copy.delete}</button></div></div></div>;
            })}
          </div>
        </section>
      )}
      ...
      {settingsOpen && (
        <div className="modal-backdrop" onMouseDown={event => event.target === event.currentTarget && setSettingsOpen(false)}>
          <section className="modal" role="dialog" aria-modal="true" aria-labelledby="settings-title">
            <div className="modal-heading"><div><span className="section-kicker">{copy.account}</span><h2 id="settings-title">{copy.settings}</h2></div><button className="icon-button" onClick={() => setSettingsOpen(false)} aria-label={copy.close}>×</button></div>
            <form onSubmit={saveProfile}>
              <div className="profile-preview">{avatarInput || profile.avatar ? <img src={avatarInput || profile.avatar || ""} alt={copy.avatar} /> : <span>{(profile.name || session.email).slice(0, 1).toUpperCase()}</span>}<label className="secondary file-button">{copy.avatar}<input type="file" accept="image/png,image/jpeg,image/webp" onChange={event => readAvatar(event.target.files?.[0])} /></label></div>
              <label className="field-label">{copy.name}<input value={profileName} onChange={event => setProfileName(event.target.value)} placeholder={copy.name} /></label>
              <label className="field-label">{copy.language}<select value={profile.language} onChange={event => void changeLanguage(event.target.value as Language)}><option value="ru">Русский</option><option value="uk">Українська</option><option value="en">English</option><option value="pl">Polski</option></select></label>
              <div className="settings-divider">{copy.changePassword}</div>
              <label className="field-label">{copy.currentPassword}<input type="password" value={currentPassword} onChange={event => setCurrentPassword(event.target.value)} /></label>
              <label className="field-label">{copy.newPassword}<input type="password" minLength={8} value={newPassword} onChange={event => setNewPassword(event.target.value)} placeholder="Minimum 8" /></label>
              {profileStatus && <p className={profileStatus === "Профиль сохранён." ? "ok" : "error"}>{profileStatus}</p>}
              <div className="modal-actions"><button type="button" className="secondary" onClick={() => setSettingsOpen(false)}>{copy.close}</button><button type="submit" className="primary">{copy.save}</button></div>
            </form>
          </section>
        </div>
      )}
